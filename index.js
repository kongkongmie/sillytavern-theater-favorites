...
function addPreviewResizeBridge(html, token) {
    ...
    const bridge = `<script>(function(){var queued=false,lastHeight=0,lastScale=1,lastNaturalWidth=0;var rgb=function(value){var match=String(value||'').match(/[\\d.]+/g);if(!match||match.length<3)return null;return match.slice(0,4).map(Number)};var visibleBackground=function(node){while(node){var color=rgb(getComputedStyle(node).backgroundColor);if(color&&color[3]!==0)return color;node=node.parentElement}return null};var adaptCanvasColor=function(){var body=document.body;var root=document.documentElement;if(!body)return;var background=visibleBackground(body);var fallback=background&&(.2126*background[0]+.7152*background[1]+.0722*background[2])>170?'#28251f':'#eee9df';root.style.color=fallback;root.style.colorScheme=background&&fallback==='#28251f'?'light':'dark';if(fallback==='#eee9df'){document.querySelectorAll('body *').forEach(function(node){var style=getComputedStyle(node);var ownBackground=rgb(style.backgroundColor);if(!ownBackground||ownBackground[3]===0)return;var luminance=.2126*ownBackground[0]+.7152*ownBackground[1]+.0722*ownBackground[2];if(luminance>170&&/238, 233, 223/.test(style.color))node.style.color='#28251f'})}};var fit=function(){var body=document.body;var root=document.documentElement;if(!body)return 1;adaptCanvasColor();var viewport=Math.max(1,root.clientWidth);var natural=Math.max(body.scrollWidth,body.offsetWidth);var scale=natural>viewport+8?viewport/natural:1;if(Math.abs(scale-lastScale)>.002||Math.abs(natural-lastNaturalWidth)>2){lastScale=scale;lastNaturalWidth=natural;if(scale<.998){body.style.transform='scale('+scale+')';body.style.width=natural+'px';root.style.overflowX='hidden'}else{body.style.transform='';body.style.width=''}}return scale};var send=function(){if(queued)return;queued=true;requestAnimationFrame(function(){queued=false;var body=document.body;var root=document.documentElement;var scale=fit();var naturalHeight=Math.max(root.scrollHeight,root.offsetHeight,body?body.scrollHeight:0,body?body.offsetHeight:0);var height=Math.ceil(naturalHeight*scale);if(Math.abs(height-lastHeight)<2)return;lastHeight=height;parent.postMessage({type:'theater-favorites-resize',token:${JSON.stringify(token)},height:height},'*')})};addEventListener('load',send);addEventListener('resize',send);new MutationObserver(send).observe(document.documentElement,{subtree:true,childList:true});if(window.ResizeObserver){var observer=new ResizeObserver(send);observer.observe(document.documentElement);if(document.body)observer.observe(document.body)}setTimeout(send,100);setTimeout(send,600)})();<\/script>`;
